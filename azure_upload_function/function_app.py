...
def _chart_type_from_query(query: str) -> str:
    """Intelligent chart type selection based on query keywords."""
    q = query.lower()
    if any(k in q for k in ("scatter", "correlation", "relationship")):
        return "scatter"
    if any(k in q for k in ("histogram", "distribution", "frequency")):
        return "histogram"
...
def query(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        top_k      = 7   # hybrid search + reranking: always retrieve 7 chunks
        ...
        # Build citations: deduplicated by filename, ordered by score
        seen_files = set()
        sources = []
        for d in docs:
